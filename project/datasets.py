...
# from torch.project.data import Dataset
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=416):
        if os.path.isdir(path):
            ...
            self.files = sorted(glob.glob('%s/*.*' % path))     # 图片路径 'data/samples\\dog-cycle-car.png'
        ...
        assert self.nF > 0, 'No images found in ' + path    # 如果没有图片，返回报错
    ...
    def __next__(self):
        self.count += 1
        if self.count == self.nF:   # 全部载入完了
            raise StopIteration     # 停止迭代
        img_path = self.files[self.count]   # 图片路径
        ...
        img, _, _, _ = letterbox(img0, height=self.height)      # 通过补灰，等比例调整成416*416

        # OpenCV 将图像载入格式是 [h, w, channel]，颜色通道的顺序为 BGR
        # PyTorch 图像格式是[batch, channel, h, w]，其通道顺序为 RGB
        # 进行格式转换
...
def letterbox(img, height=416, color=(127.5, 127.5, 127.5)):  # resize a rectangular image to a padded square
    ...
    #classfier = cv2.CascadeClassifier(r"D:\python\Lib\site-packages\cv2\data\haarcascade_frontalface_default.xml")
